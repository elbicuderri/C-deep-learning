	...
	std::vector<float> Weight, std::vector<float> Bias)
{
	...
	for (int n = 0; n < N; n++) {
		for (int k = 0; k < K; k++) {
			...
			for (int c = 0; c < C; c++) {
				int Input_index = n * C + c;
				int Weight_index = c * K + k;  // Torch와 이 부분의 index만 다름
				float s = Input[Input_index] * Weight[Weight_index];
				sum += s;
			}
			sum += Bias[k];
...
std::vector<float> Softmax(std::vector<float> Tensor, int Classes)
{
	...
	for (int i = 0; i < OutputSize; i++) {
		int p = i / C;
		Output[i] = expf(Tensor[i]) / ExponentialSum(std::vector<float>(Tensor.begin() + p * C, Tensor.begin() + (p * C) + C), C);
	}
...
int main()
{
	const int N = 100; // 0 ~ N번째의 mnist test 이미지

	std::vector<float> W1{ LoadData("weight_torch/W1_torch.wts") }; // 784 * 16
	std::vector<float> b1{ LoadData("weight_torch/b1_torch.wts") }; // 16

	std::vector<float> W2{ LoadData("weight_torch/W2_torch.wts") }; // 16 * 16
	std::vector<float> b2{ LoadData("weight_torch/b2_torch.wts") }; // 16 

	std::vector<float> W3{ LoadData("weight_torch/W3_torch.wts") }; // 16 * 16
	std::vector<float> b3{ LoadData("weight_torch/b3_torch.wts") }; // 16 

	std::vector<float> W4{ LoadData("weight_torch/W4_torch.wts") }; // 16 * 16
	std::vector<float> b4{ LoadData("weight_torch/b4_torch.wts") }; // 16 

	std::vector<float> W5{ LoadData("weight_torch/W5_torch.wts") }; // 16 * 16
	std::vector<float> b5{ LoadData("weight_torch/b5_torch.wts") }; // 16 

	std::vector<float> W6{ LoadData("weight_torch/W6_torch.wts") }; // 16 * 10
	std::vector<float> b6{ LoadData("weight_torch/b6_torch.wts") }; // 10

	std::vector<float> image{ LoadData("data/mnist_test_float32.bin") }; // 10000개의 mnist test 이미지
	...
	std::vector<float> logit_origin{ LoadData("value_torch/logit_torch.layers") }; //logit 비교
	...
	std::vector<float> relu_dense4_origin{ LoadData("value_torch/relu_dense4_torch.layers") }; //중간 레이어들도 비교
