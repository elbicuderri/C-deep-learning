	...
	const std::vector<float>& Weight, const std::vector<float>& Bias)
{
	...
	for (int n = 0; n < N; n++) {
		for (int k = 0; k < K; k++) {
			...
			for (int c = 0; c < C; c++) {
				int Input_index = n * C + c;
				int Weight_index = c * K + k;  // Torch�� �� �κ��� index�� �ٸ�
				float s = Input[Input_index] * Weight[Weight_index];
				sum += s;
			}
			sum += Bias[k];
...
int main()
{
	const int N = 100; // 0 ~ N��°�� mnist test �̹���
	...
	std::vector<float> image{ LoadData("data/mnist_test_float32.bin") }; // 10000���� mnist test �̹���
	...
	std::vector<float> logit_origin{ LoadData("value_torch/logit_torch.layers") }; //logit ��
	...
	std::vector<float> relu_dense4_origin{ LoadData("value_torch/relu_dense4_torch.layers") }; //�߰� ���̾�鵵 ��
