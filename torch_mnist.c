...
int main()
{
	...
	const char* data_file = "C:\\data\\mnist_test_float.bin";
	...
	load_data(kernel, "weight/conv.weight_pytorch.bin", K * C * kH * kW);

	float *gamma = (float*)malloc(K * sizeof(float));
	load_data(gamma, "weight/batchnorm.weight_pytorch.bin", K);

	float *beta = (float*)malloc(K * sizeof(float));;
	load_data(beta, "weight/batchnorm.bias_pytorch.bin", K);
	...
	load_data(W1, "weight/dense1.weight_pytorch.bin", K * maxpool_H * maxpool_W * dense1_units);

	float *b1 = (float*)malloc(dense1_units * sizeof(float));;
	load_data(b1, "weight/dense1.bias_pytorch.bin", dense1_units);

	float *W2 = (float*)malloc(dense1_units * dense2_units * sizeof(float));;
	load_data(W2, "weight/dense2.weight_pytorch.bin", dense1_units * dense2_units);

	float *b2 = (float*)malloc(dense2_units * sizeof(float));;
	load_data(b2, "weight/dense2.bias_pytorch.bin", dense2_units);
	...
	int stride_height, int stride_width)
{
	...
	//set weight (conv weight, bias + batchnorm weight, bias, mean, variance, epsilon)
	...
	//set bias (conv weight, bias(here None) + batchnorm weight, bias, mean, variance, epsilon)
...
void maxpooling(float *output, float *input,
 int batch, int channel, int input_height, int input_width,
  int kernel_height, int kernel_width,
   int pad_top, int pad_bottom, int pad_left, int pad_right,
    int stride_height, int stride_width)
{
...
void softmax(float *output, float *input, int batch, int classes)
{
	int N = batch;
	int C = classes;

	for (int i = 0; i < N; i++) {
		float exp_sum = exponential_sum(input, C, i * C);
		for (int j = 0; j < C; j++) {
			int index = i * C + j;
...
void softmax_v2(float* output, float* input, int batch, int classes)
{
	...
	int C = classes;
	// N : batch, C : classes
	for (int id = 0; id < N*C; id++) {
		int p = id / C; // p번째 클래스에 대한 확률
		float sum = (float)0.0f;
		for (int c = 0; c < C; c++) {
			float element = input[p * C + c]; 
			float element_exponential = expf(element); 
			sum += element_exponential; // sigma(e^x) 구하는 과정
		}

		output[id] = expf(input[id]) / sum; // p번째 클래스에 대한 확률 대입
	}
