...
class mnist_model(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(1, 5, kernel_size=5, padding=2, stride=2, bias=False)
        self.batchnorm = nn.BatchNorm2d(5, eps=0.001)
        self.maxpool = nn.MaxPool2d(2, stride=2)
        self.dense1 = nn.Linear(5 * 7 * 7, 120)
    ...
    def forward(self, x):
        ...
        if self.mode == 1:
            def save_value(value, name):
                ...
                value_arr.tofile(f"value/{name}_pytorch_2.bin")
...
def save_weights(weights, name):
    ...
    weights.tofile(f"weight/{name}_pytorch_2.bin")
