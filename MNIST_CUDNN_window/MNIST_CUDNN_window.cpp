...
int main()
{
	...
	cudnnStatus_t error_create_biasDesc = cudnnCreateTensorDescriptor(&biasDesc);
	if (error_create_biasDesc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_biasDesc = cudnnSetTensor4dDescriptor(
		biasDesc,
		dataformat,
		datatype,
		1,
		...
		1,
		1);
	if (error_set_biasDesc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnAddTensor = cudnnAddTensor(
		HANDLE,
		&alpha_bias,
		biasDesc,
		d_bias,
		...
		conv_imageDesc,
		d_conv_fusion);
	if (error_cudnnAddTensor != CUDNN_STATUS_SUCCESS) {
		printf("cudnnAddTensor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnCreatePoolingDescriptor = cudnnCreatePoolingDescriptor(&poolingDesc);
	if (error_cudnnCreatePoolingDescriptor != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreatePoolingDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_cudnnSetPooling2dDescriptor = cudnnSetPooling2dDescriptor(
		poolingDesc,
		pooling_mode,
		maxpoolingNanOpt,
		maxpool_kH,
		...
		maxpool_sH,
		maxpool_sW);
	if (error_cudnnSetPooling2dDescriptor != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetPooling2dDescriptor error!\n");
		exit(-1);
	}

	cudnnTensorDescriptor_t maxpoolDesc;

	cudnnStatus_t error_create_maxpoolDesc = cudnnCreateTensorDescriptor(&maxpoolDesc);
	if (error_create_maxpoolDesc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_maxpoolDesc = cudnnSetTensor4dDescriptor(
		maxpoolDesc,
		dataformat,
		datatype,
		N,
		...
		maxpool_H,
		maxpool_W);
	if (error_set_maxpoolDesc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnPoolingForward = cudnnPoolingForward(
		HANDLE,
		poolingDesc,
		&alpha_pool,
		conv_imageDesc,
	...
	if (error_cudnnPoolingForward != CUDNN_STATUS_SUCCESS) {
		printf("cudnnPoolingForward error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_create_activationDesc = cudnnCreateActivationDescriptor(&activationDesc);
	if (error_create_activationDesc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateActivationDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_activationDesc = cudnnSetActivationDescriptor(
		activationDesc,
		activation_mode,
		reluNanOpt,
		coef);
	if (error_set_activationDesc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetActivationDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_cudnnActivationForward = cudnnActivationForward(
		HANDLE,
		activationDesc,
		&alpha_activation,
		maxpoolDesc,
		...
		maxpoolDesc,
		d_relu_maxpool);
	if (error_cudnnActivationForward != CUDNN_STATUS_SUCCESS) {
		printf("cudnnActivationForward error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_create_x_dense1Desc = cudnnCreateTensorDescriptor(&x_dense1Desc);
	if (error_create_x_dense1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_x_dense1Desc = cudnnSetTensor4dDescriptor(
		x_dense1Desc,
		dataformat,
		datatype,
		N,
		...
		1,
		1);
	if (error_set_x_dense1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}

	cudnnFilterDescriptor_t W1Desc;
	cudnnStatus_t error_create_W1Desc = cudnnCreateFilterDescriptor(&W1Desc);
	if (error_create_W1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateFilterDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_W1Desc = cudnnSetFilter4dDescriptor(
		W1Desc,
		datatype,
		dataformat,
		dense1_units,
		...
		1,
		1);
	if (error_set_W1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetFilter4dDescriptor error!\n");
		exit(-1);
	}

	cudnnConvolutionDescriptor_t dense1Desc;
	cudnnStatus_t error_create_dense1Desc = cudnnCreateConvolutionDescriptor(&dense1Desc);
	if (error_create_dense1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateConvolutionDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_dense1Desc = cudnnSetConvolution2dDescriptor(
		dense1Desc,
		0,
		0,
		1,
		...
		conv_mode,
		datatype);
	if (error_set_dense1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetConvolution2dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnGetConvolution2dForwardOutputDim = cudnnGetConvolution2dForwardOutputDim(
		dense1Desc,
		x_dense1Desc,
		W1Desc,
		&dense1_N,
		...
		&dense1_H,
		&dense1_W);
	if (error_cudnnGetConvolution2dForwardOutputDim != CUDNN_STATUS_SUCCESS) {
		printf("cudnnGetConvolution2dForwardOutputDim error!\n");
		exit(-1);
	}

	cudnnTensorDescriptor_t y_dense1Desc;
	cudnnStatus_t error_create_y_dense1Desc = cudnnCreateTensorDescriptor(&y_dense1Desc);
	if (error_create_y_dense1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_y_dense1Desc = cudnnSetTensor4dDescriptor(
		y_dense1Desc,
		dataformat,
		datatype,
		dense1_N,
		...
		dense1_H,
		dense1_W);
	if (error_set_y_dense1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnGetConvolutionForwardAlgorithm_dense1 = cudnnGetConvolutionForwardAlgorithm(
		HANDLE,
		x_dense1Desc,
		W1Desc,
		dense1Desc,
		...
		&algo_dense1);
	if (error_cudnnGetConvolutionForwardAlgorithm_dense1 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnGetConvolutionForwardAlgorithm error!\n");
		exit(-1);
	}

	size_t sizeInBytes_dense1;
	cudnnStatus_t error_cudnnGetConvolutionForwardWorkspaceSize_dense1 = cudnnGetConvolutionForwardWorkspaceSize(
		HANDLE,
		x_dense1Desc,
		W1Desc,
		dense1Desc,
		...
		algo_dense1,
		&sizeInBytes_dense1);
	if (error_cudnnGetConvolutionForwardWorkspaceSize_dense1 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnGetConvolutionForwardWorkspaceSize error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_dense1 = cudnnConvolutionForward(
		HANDLE,
		&alpha_dense1,
		x_dense1Desc,
		d_relu_maxpool,
		...
		y_dense1Desc,
		d_dense1);
	if (error_dense1 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnConvolutionForward error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_create_b1Desc = cudnnCreateTensorDescriptor(&b1Desc);
	if (error_create_b1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_b1Desc = cudnnSetTensor4dDescriptor(
		b1Desc,
		dataformat,
		datatype,
		1,
		...
		dense1_H,
		dense1_W);
	if (error_set_b1Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnAddTensor_dense1 = cudnnAddTensor(
		HANDLE,
		&alpha_bias_dense1,
		b1Desc,
		d_b1,
		...
		y_dense1Desc,
		d_dense1);
	if (error_cudnnAddTensor_dense1 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnAddTensor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_create_activationDesc_dense1 = cudnnCreateActivationDescriptor(&activationDesc_dense1);
	if (error_create_activationDesc_dense1 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateActivationDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_activationDesc_dense1 = cudnnSetActivationDescriptor(
		activationDesc_dense1,
		activation_mode_dense1,
		reluNanOpt_dense1,
		coef_dense1);
	if (error_set_activationDesc_dense1 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetActivationDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_cudnnActivationForward_dense1 = cudnnActivationForward(
		HANDLE,
		activationDesc_dense1,
		...
		y_dense1Desc,
		d_relu_dense1);
	if (error_cudnnActivationForward_dense1 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnActivationForward error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_create_x_dense2Desc = cudnnCreateTensorDescriptor(&x_dense2Desc);
	if (error_create_x_dense2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_x_dense2Desc = cudnnSetTensor4dDescriptor(
		x_dense2Desc,
		dataformat,
		datatype,
		dense1_N,
		...
		dense1_H,
		dense1_W);
	if (error_set_x_dense2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}

	cudnnFilterDescriptor_t W2Desc;
	cudnnStatus_t error_create_W2Desc = cudnnCreateFilterDescriptor(&W2Desc);
	if (error_create_W2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateFilterDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_W2Desc = cudnnSetFilter4dDescriptor(
		W2Desc,
		datatype,
		dataformat,
		dense2_units,
		...
		1,
		1);
	if (error_set_W2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetFilter4dDescriptor error!\n");
		exit(-1);
	}

	cudnnConvolutionDescriptor_t dense2Desc;
	cudnnStatus_t error_create_dense2Desc = cudnnCreateConvolutionDescriptor(&dense2Desc);
	if (error_create_dense2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateConvolutionDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_dense2Desc = cudnnSetConvolution2dDescriptor(
		dense2Desc,
		0,
		0,
		1,
		...
		conv_mode,
		datatype);
	if (error_set_dense2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetConvolution2dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnGetConvolution2dForwardOutputDim_dense2 = cudnnGetConvolution2dForwardOutputDim(
		dense2Desc,
		x_dense2Desc,
		W2Desc,
		&dense2_N,
		...
		&dense2_H,
		&dense2_W);
	if (error_cudnnGetConvolution2dForwardOutputDim_dense2 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnGetConvolution2dForwardOutputDim error!\n");
		exit(-1);
	}

	cudnnTensorDescriptor_t y_dense2Desc;
	cudnnStatus_t error_create_y_dense2Desc = cudnnCreateTensorDescriptor(&y_dense2Desc);
	if (error_create_y_dense2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_y_dense2Desc = cudnnSetTensor4dDescriptor(
		y_dense2Desc,
		dataformat,
		datatype,
		dense2_N,
		...
		dense2_H,
		dense2_W);
	if (error_set_y_dense2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnGetConvolutionForwardAlgorithm_dense2 = cudnnGetConvolutionForwardAlgorithm(
		HANDLE,
		x_dense2Desc,
		W2Desc,
		dense2Desc,
		...
		&algo_dense2);
	if (error_cudnnGetConvolutionForwardAlgorithm_dense2 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnGetConvolutionForwardAlgorithm error!\n");
		exit(-1);
	}

	size_t sizeInBytes_dense2;
	cudnnStatus_t error_cudnnGetConvolutionForwardWorkspaceSize_dense2 = cudnnGetConvolutionForwardWorkspaceSize(
		HANDLE,
		x_dense2Desc,
		W2Desc,
		dense2Desc,
		...
		algo_dense2,
		&sizeInBytes_dense2);
	if (error_cudnnGetConvolutionForwardWorkspaceSize_dense2 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnGetConvolutionForwardWorkspaceSize error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t err_dense2 = cudnnConvolutionForward(
		HANDLE,
		&alpha_dense2,
		x_dense2Desc,
		d_relu_dense1,
		...
		y_dense2Desc,
		d_dense2);
	if (err_dense2 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnConvolutionForward error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_create_b2Desc = cudnnCreateTensorDescriptor(&b2Desc);
	if (error_create_b2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnCreateTensorDescriptor error!\n");
		exit(-1);
	}

	cudnnStatus_t error_set_b2Desc = cudnnSetTensor4dDescriptor(
		b2Desc,
		dataformat,
		datatype,
		1,
		...
		dense2_H,
		dense2_W);
	if (error_set_b2Desc != CUDNN_STATUS_SUCCESS) {
		printf("cudnnSetTensor4dDescriptor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnAddTensor_dense2 = cudnnAddTensor(
		HANDLE,
		&alpha_bias_dense2,
		b2Desc,
		d_b2,
		...
		y_dense2Desc,
		d_dense2);
	if (error_cudnnAddTensor_dense2 != CUDNN_STATUS_SUCCESS) {
		printf("cudnnAddTensor error!\n");
		exit(-1);
	}
	...
	cudnnStatus_t error_cudnnSoftmaxForward = cudnnSoftmaxForward(
		HANDLE,
		algorithm_softmax,
		mode_softmax,
